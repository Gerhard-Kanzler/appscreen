// Three.js 3D Renderer for iPhone / Samsung mockups
...
// Replace the default "glass" mesh material with deep black so any sub-pixel
// gap between our custom screen plane and the bezel cutout doesn't show as
// a light frame. Called for every model load path.
function darkenScreenGlass(model) {
  if (!model) return;
...
function initThreeJS() {
  ...
  if (
    ...
    typeof getScreenshotSettings === "function"
  ) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      if (glassMeshes.length > 0) {
        ...
        console.log(
          "  -> Using largest glass mesh as screen:",
          screenMesh.name,
        );
      }

      // Darken default glass so it never shows through as a light frame
      darkenScreenGlass(phoneModel);
...
function switchPhoneModel(deviceType) {
  ...
  if (
    ...
    (phoneModelLoaded || phoneModelLoading)
  ) {
    return;
  }
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      // Darken default glass so it never shows as a light frame
      darkenScreenGlass(phoneModel);
      ...
      if (typeof state !== "undefined") {
        ...
        if (
          typeof updateCanvas === "function" &&
          !window.suppressSwitchModelUpdate
        ) {
          updateCanvas();
        }
      }
...
function loadCachedPhoneModel(deviceType) {
  ...
  phoneModelCache[deviceType].loadingPromise = new Promise(
    (resolve, reject) => {
      loader.load(
        config.modelPath,
        (gltf) => {
          ...
          // Darken default glass so it never shows as a light frame
          darkenScreenGlass(model);

          // Create screen plane for this model (match createScreenOverlay)
        ...
        (error) => {
          console.error(
            "Error loading cached " + deviceType + " model:",
            error,
          );
...
function preloadAllPhoneModels() {
  ...
  return Promise.all(
    deviceTypes.map((type) => loadCachedPhoneModel(type).catch(() => null)),
  );
}
...
function createScreenOverlay() {
  ...
  // Plane size matches the screen aspect ratio exactly. The rim mesh
  // (Material.001) is darkened in darkenScreenGlass(), so no edge tricks.
  const aspectRatio = config.aspectRatio;
  const planeHeight = 4.3 * config.screenHeightFactor;
  const planeWidth = planeHeight * aspectRatio;
  ...
  const screenOffset = config.screenOffset;
  customScreenPlane.position.set(
    screenOffset.x,
    screenOffset.y,
    screenOffset.z,
  );
  ...
  console.log(
    "Created screen overlay for " + currentDeviceModel + " at:",
    customScreenPlane.position,
  );
  console.log(
    "Plane size:",
    planeWidth.toFixed(4),
    "x",
    planeHeight.toFixed(4),
  );
}

// Create a rounded-corner version of the screenshot.
// Draw image, then mask with a rounded rectangle via destination-in.
// Combined with the material's alphaTest, the rounded edge is crisp; the
// darkened rim mesh behind hides any sub-pixel imprecision.
...
function updateScreenTexture() {
  ...
  const screenshotImage =
    typeof getScreenshotImage === "function"
  ...
  const cornerRadius = Math.round(
    screenshotImage.width * config.cornerRadiusFactor,
  );
  ...
  screenTexture.premultiplyAlpha = false; // canvas data is straight alpha
  screenTexture.minFilter = THREE.LinearFilter;
  screenTexture.magFilter = THREE.LinearFilter;
  screenTexture.generateMipmaps = false; // no mip-blending at edges

  // Material — alphaTest gives a hard cut at 50% alpha, eliminating AA
  // transition pixels. Combined with the inset rounded mask and the dark
  // glass behind, the edge is sharp and invisible.
...
function renderThreeJSForScreenshot(
  ...
  if (typeof state === "undefined" || !state.screenshots[screenshotIndex])
    return;
  ...
  const useCurrentModel =
    screenshotDeviceType === currentDeviceModel && phonePivot;
